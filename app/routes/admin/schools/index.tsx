...
import { findMany } from "~/api/schools/read";
...
// A nested route for the Schools route
export default function SchoolIndex() {
  ...
  const [selectedRows, setSelectedRows] = useState<Row<School>[]>([]);
  // use this handle to remove an school from the table, it will not remove the record from the database, it will change the record deleted field to true.
  ...
  // table columns for the school table
  ...
  let onDelete = useCallback(() => {
    removeHandle.submit(
      ...
      { action: "/api/schools", method: "delete" }
    );
