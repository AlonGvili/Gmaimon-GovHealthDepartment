...
import { findMany } from "~/api/devices/read";
...
// A nested route for the Devices route
export default function DeviceIndex() {
  ...
  const [selectedRows, setSelectedRows] = useState<Row<Device>[]>([]);
  // use this handle to remove an device from the table, it will not remove the record from the database, it will change the record deleted field to true.
  ...
  // table columns for the device table
  ...
  let onDelete = useCallback(() => {
    removeHandle.submit(
      ...
      { action: "/api/devices", method: "delete" }
    );
